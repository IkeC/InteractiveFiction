...
[ TM_MSG_TALK_ABOUT_WHAT;
	print "What would you like to say to ", (the) current_talker, "?^";
];
#Endif;
#Ifndef TM_MSG_TOPICS_DEPLETED;
Constant TM_MSG_TOPICS_DEPLETED "^With that, you politely end the conversation.";
#Endif;
#Ifndef TM_MSG_EXIT;
Constant TM_MSG_EXIT TM_REUSABLE_MSG_EXIT;
#Endif;
#Ifndef TM_MSG_NO_TOPICS;
Constant TM_MSG_NO_TOPICS "You wouldn't know what to talk about right now.";
#Endif;
#Ifndef TM_MSG_EXIT_OPTION;
Constant TM_MSG_EXIT_OPTION "[0] End conversation^";
#Endif;
...
#Ifnot;
[ RunTalk p_npc _array _i _j _n _val _tm_start _add_msg _count _shown_header;
#Endif;
...
#Ifnot;
			_n++;
!			print "Talk to ", (the) p_npc, " about:^";
			if(_n == 1 && _shown_header == 0) { _TMPrintMsg(TM_MSG_TALK_ABOUT_WHAT); _shown_header = 1; }
			if(_n == 1) print "^";
			print "[", _n, "] ";
#Endif;
			_i++;
...
#Endif;
		...
		! Topics exhausted after conversation - show [0] and wait
	}
...
!		print "_i is ", _i, ", _val is ", _val, "^";
		if(_val == TM_ACTIVE) {
			...
			_count = _i; ! Save label index for topic echo
			_i++;
#Ifv3;
			@set_window 0;
#Endif;
...
!		print "Performing action ", _val, " at pos ", _j, "^";
		...
		if(_val > 0) {
			...
			if(_val <= TM_LAST_ID) {
				...
				!IkeC: ReActivate so we can keep the current topic active
