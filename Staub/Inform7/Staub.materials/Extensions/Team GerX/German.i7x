...
The description of yourself is usually "Du wirkst etwas mitgenommen."
...
Understand "Nord", "Norden" as north.
The printed name of north is "Norden[-s][m]".

Understand "Nordost", "Nordosten", "no" as northeast.
The printed name of northeast is "Nordosten[-s][m]".

Understand "Nordwest", "Nordwesten", "nw" as northwest.
The printed name of northwest is "Nordwesten[-s][m]".

Understand "Sued","Sueden" as south.
The printed name of south is "Süden[-s][m]".

Understand "Suedost", "Suedosten", "so" as southeast.
The printed name of southeast is "Südosten[-s][m]".

Understand "Suedwest", "Suedwesten" as southwest.
The printed Name of southwest is "Südwesten[-s][m]".

Understand "Ost", "Osten" or "o" as east.
The printed name of east is "Osten[-s][m]".

Understand "West", "Westen" as west.
...
Understand "raus", "aussen", "draussen" as outside.
...
Understand the commands "renn", "wander", "fluecht", "flieh", "schreit", and "spazier" as "geh".
...
Understand the commands "i", "inv", "inf", "besitz" and "eigentum" as "inventar".
...
Understand the commands "seh" and "sieh" and "blick" and "lug" and "guck" and "kuck" and "l" as "schau".

Understand "lage" as looking.
...
Understand "leg [things preferably held] auf boden" as dropping.
Understand "leg [things preferably held] auf den boden" as dropping.
...
Understand "unt [something]" as examining.
...
Understand the commands "x", "betracht", "beschreib", "begutacht", and "inspizier" as "untersuch".
...
Understand "zieh [something] [noun heraus]" as pulling.
...
Understand "drueck [something] an/zu/unter [something]" as using it on.
Understand the commands "press", "schieb" and "verschieb" as "drueck".
...
Understand the commands "attackier", "ermord", "mord", "bekaempf", "folter", "quael", "pruegle", "ueberwaeltig" and "pruegel" as "toet".
...
Understand "red [dativ] [someone alive]" as telling it about. [*]
Understand "red [dativ] [something]" as telling it about. [*]
...
Understand the commands "sprech" and "sprich" and "schwatz" and "schwaetz" and "troest" as "red".
...
Understand "kletter [herunter]" as climbing. [ +++ 23.04.2023 "runter/herunter" durch Token ersetzt ]
...
Understand the commands "kost" and "probier" as "schmeck".
...
Check vaguely communicating (this is the block vaguely communicating rule):
	say "Bitte gib auch an, mit wem [du] reden möchtest.".
...
Carry out location-leaving (this is the leave current location rule):
	...
	if N is greater than 1, say "Es gibt mehrere Ausgänge. Bitte sage genau, wohin [du] gehen möchtest. Möglicherweise hilft dir INFO weiter." (C);
...
[ IsDirectionWord w	obj i index;
	...
	switch (w) {
		...
		'nordost', 'nordosten', 'no', 'ne': return (+ northeast +);
		'nordwest', 'nordwesten', 'nw': return (+ northwest +);
		'sued', 'sueden', 's//': return (+ south +);
		'suedost', 'suedosten', 'so', 'se': return (+ southeast +);
		'suedwest', 'suedwesten', 'sw': return (+ southwest +);
		'ost', 'osten', 'o//', 'e//': return (+ east +);
		'west', 'westen', 'w//': return (+ west +);
...
The parser error internal rule response (A) is "[one of]Diesen Satz habe ich leider nicht verstanden.[or]Ich verstehe kein Wort.[or]Ich habe keine Ahnung was du willst.[or]Das funktioniert so leider nicht.[or]Das ergibt für mich überhaupt keinen Sinn.[or]Das ist bestimmt eine gute Idee, aber ich verstehe leider rein gar nichts.[or]So läuft das hier nicht.[or]Das verstehe ich leider nicht.[or]Das funktioniert so leider nicht.[or]Tut mir leid, das verstehe ich nicht.[or]Das habe ich nicht verstanden. Möglicherweise hilft dir INFO weiter.[at random]".
	[was "I didn't understand that sentence."]
The parser error internal rule response (B) is "Möglicherweise hilft dir INFO weiter. Ich habe nur Folgendes verstanden: ".
	[was "I only understood you as far as wanting to "]
The parser error internal rule response (C) is "Möglicherweise hilft dir INFO weiter. Ich habe nur verstanden, dass [du] irgendwohin gehen möchtest ".
...
The parser error internal rule response (X) is "[one of]Wie bitte?[or]Was?[or]Wie meinen?[or]Pardon?[at random]".
	[was "I beg your pardon?"]

The parser nothing error internal rule response (A) is "Es gibt hier nichts zu tun!".
	[was "Nothing to do!"]
The parser nothing error internal rule response (B) is "Du musst schon etwas genauer sagen was [du] machen willst.".
...
The standard report taking rule response (A) is "[one of]In Ordnung.[or]Du nimmst [den noun] an dich.[at random]".
	[was "Taken."]
...
The block attacking rule response (A) is "Gewalt ist keine Lösung - zumindest jetzt gerade nicht.".
...
The block kissing rule response (A) is "[Der noun] [mag] das [one of]vermutlich[or]wahrscheinlich[at random] nicht.".
	[was "[The noun] [might not] like that."]

The block answering rule response (A) is "Rede einfach mit [dem noun].".
	[was "[There] [are] no reply."]

The telling yourself rule response (A) is "[one of]Du unterhältst [dich] ein wenig und erzählst [dir] alte Geschichten.[or]Du summst etwas vor dich hin.[or][italic type]Ich finde dich Claire.[at random]".
