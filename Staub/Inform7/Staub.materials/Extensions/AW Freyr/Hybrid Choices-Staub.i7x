...
"Provides a choice-based interface that can be freely switched between interactive and non-interactive modes. Basically, it does CYOA. (modded for Staub)"
...
To execute page turning:
	...
	while the finish condition is false:
		...
		if the current page flips to a page (called p1):
			...
			follow the page-toggle rules for the current page;
			[IkeC Mod]
			increase Toldcounter of the current page by 1;
			if the current page is a dead-end:
				...
				if the current page is an end-page:
					now finish condition is true;
				else:
					now the current page is the previous page;
					follow the page-toggle rules for the current page;
					follow the page-output rules for the current page;
					unless the current page offers non-end choices:
...
A page has a number called Toldcounter. The Toldcounter of a page is usually 0.
To say page-told: 
	if Toldcounter of the current page is 2:
		say " Das habe ich dir doch schon gesagt![run paragraph on]";
	else if Toldcounter of the current page is greater than 15:
		say " Das habe ich dir jetzt schon [bold type]VIEL ZU OFT[roman type] gesagt![run paragraph on]";
	else if Toldcounter of the current page is greater than 12:
		say " Das habe ich dir jetzt schon sehr, [italic type]SEHR[roman type] oft gesagt![run paragraph on]";
	else if Toldcounter of the current page is greater than 2:
		say " Das habe ich dir doch schon [Toldcounter in words]mal [one of]gesagt[or]erzählt[or]mitgeteilt[at random]![run paragraph on]";
...
To decide whether (n - a page) offers non-end choices:
	repeat with K running through pages turned to by N:
		follow the choice-switch rules for K;
		unless rule failed:
			unless K is an end-page:
				decide yes;
	decide no.
...
Rule for displaying available decisions of a page (called N) (this is the display all choices rule):
	...
	repeat through the table of currently available choices:
		if decision entry is an end-page and "[decision entry]" matches the text "-end":
			say "[bracket]0[close bracket]";
		else:
			say "[bracket][index entry][close bracket]";
		say " " (A);
...
Check choosing (this is the can't choose a number for a non-existent choice rule):
	if the number understood is 0:
		[IkeC: set the number to the last number of the list]
		repeat through the table of currently available choices:
			if the number understood is less than index entry:
				now the number understood is index entry;
	else if the number understood is not an index listed in the table of currently available choices:
		say "Bitte wähle eine der angezeigten Auswahlmöglichkeiten oder 0, um zu beenden." (A) instead.
...
The CYOA error message is a text that varies. The CYOA error message is "Bitte wähle eine der angezeigten Auswahlmöglichkeiten oder 0, um zu beenden."
