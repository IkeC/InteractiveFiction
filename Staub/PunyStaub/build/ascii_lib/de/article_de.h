...
! fuer alle drei Kasus (Nominativ, Akkusativ, Dativ) und alle vier
...
! Artikel fuer alle Kasus:
!
!   Object -> Kiste "Seekiste"          ! hat female → Artikel "eine/einer/die/der..."
!       ...
!       has female, container, openable;   ! kein article: Eigenschaft noetig
!
!   Object -> Schluessel "kleiner Schluessel"   ! kein female/neuter → maskulin
!       ...                                   ! kein article: Eigenschaft noetig
!
...
!
!   Object Hotel "Hotel"
!       with article "so ein",   ! Ungewoehnliche Form
!       has neuter;
!
! === Fuer Nachrichten in messages_de.h ===
!
! Verwende folgende Druckfunktionen fuer grammatisch korrekte Kasus:
!
!   (DE_Der) obj    Nominativ bestimmt   : "der/die/das/die Schluessel"
!   (DE_Den) obj    Akkusativ bestimmt   : "den/die/das/die Schluessel"
!   (DE_Dem) obj    Dativ bestimmt       : "dem/der/dem/den Schluessel"
!   (DE_Ein) obj    Nominativ unbestimmt : "ein/eine/ein/einige Schluessel"
!   (DE_Einen) obj  Akkusativ unbestimmt : "einen/eine/ein/einige Schluessel"
!   (DE_Einem) obj  Dativ unbestimmt     : "einem/einer/einem/einigen Schluessel"
!
! (DE_Der_Cap) ist der explizite Ersatz fuer (The) am Satzanfang.
! (the)/(The) sollen in messages_de.h nicht mehr verwendet werden.
!
! === Speicherbedarf ===
! DE_DefArticles:   12 Eintraege × 2 Bytes = 24 Bytes (Zeiger auf Strings)
! DE_IndefArticles: 12 Eintraege × 2 Bytes = 24 Bytes
...
! ---------------------------------------------------------------------------
! Endungen fuer Stamm-Artikel (article "dein", "mein", "kein" usw.).
...
! ---------------------------------------------------------------------------
! _DE_IndefArtStr -- gibt den richtigen unbestimmten Artikel als String zurueck,
!   unter Beruecksichtigung einer optionalen article-Override-Eigenschaft.
!
!   Gibt 0 zurueck wenn kein Artikel gedruckt werden soll.
!   Gibt -1 zurueck wenn der Plural-Fallback "einige"/"einigen" verwendet wird.
...
[ _DE_IndefArtStr p_obj p_case p_plural_dative   g s;
    ...
    ! Override: article "string" → Stamm, wird fallabhaengig dekliniert (→ -3)
    ! Hinweis: article 'word' (Hochkomma, dict-Wort) wird NICHT unterstuetzt.
    ...
    ! im selben Wertebereich). Verwende article "string" (Anfuehrungszeichen).
...
! ---------------------------------------------------------------------------
! Bestimmte Artikel + Name (Druckfunktionen fuer Spielnachrichten)
! ---------------------------------------------------------------------------

! Nominativ bestimmt: "der Schluessel / die Kiste / das Fernrohr / die Muenzen"
! (entspricht (the) in Nominativ-Position)
[ DE_Der p_obj;
    _DE_PrintDefWithName(p_obj, Nom, false);
];

! Akkusativ bestimmt: "den Schluessel / die Kiste / das Fernrohr / die Muenzen"
[ DE_Den p_obj;
    _DE_PrintDefWithName(p_obj, Akk, false);
];

! Dativ bestimmt: "dem Schluessel / der Kiste / dem Fernrohr / den Muenzen"
[ DE_Dem p_obj;
    _DE_PrintDefWithName(p_obj, Dat, false);
];

! Nominativ bestimmt, grossgeschrieben: "Der Schluessel / Die Kiste / Das Fernrohr"
! Expliziter Ersatz fuer (The) am Satzanfang in messages_de.h
[ DE_Der_Cap p_obj;
    _DE_PrintDefWithName(p_obj, Nom, true);
];

! ---------------------------------------------------------------------------
! Unbestimmte Artikel + Name (Druckfunktionen fuer Spielnachrichten)
! ---------------------------------------------------------------------------

! Nominativ unbestimmt: "ein Schluessel / eine Kiste / ein Fernrohr / einige Muenzen"
[ DE_Ein p_obj;
    _DE_PrintIndefWithName(p_obj, Nom);
];

! Akkusativ unbestimmt: "einen Schluessel / eine Kiste / ein Fernrohr / einige Muenzen"
[ DE_Einen p_obj;
    _DE_PrintIndefWithName(p_obj, Akk);
];

! Dativ unbestimmt: "einem Schluessel / einer Kiste / einem Fernrohr / einigen Muenzen"
[ DE_Einem p_obj;
...
! Inform 6 loest Vorwaerts-Funktionsreferenzen auf.
!
! Unterstuetzte Ersetzungen:
...
!
! Rueckgabe: true wenn Ersetzungen vorgenommen wurden, sonst false.
! ---------------------------------------------------------------------------
[ _DE_SubstitutePronouns   _nwords _i _waddr _woff _wlen _blen _j _changed;
    _nwords  = parse->1;
    _changed = false;
    ! Von rechts nach links: Pufferverschiebungen bei frueheren Woertern bleiben gueltig
    for (_i = _nwords : _i >= 1 : _i--) {
        if (parse-->(_i + _i - 1) ~= 0) continue;  ! Wort im Woerterbuch → ueberspringen
        ...
        if (_wlen == 2 && _waddr->0 == 'e' && _waddr->1 == 'r') {
            if (_blen >= buffer->0) continue;  ! Pufferueberlauf verhindern
        ...
        if (_wlen == 3 && _waddr->0 == 's' && _waddr->1 == 'i' && _waddr->2 == 'e') {
            ...
            ! Weder herobj noch themobj gesetzt: "sie" unveraendert lassen
            continue;
        }

        ! --- "ihnen" (5 Bytes) → "them" (4 Bytes): ueberschreiben + 1 Byte links ---
