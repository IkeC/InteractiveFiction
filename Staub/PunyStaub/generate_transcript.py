...
def _clean_output(text: str, command: str | None = None) -> str:
    cleaned = re.sub(r"(?m)^>\s*$", "", text)
    cleaned = re.sub(r"(?m)^>\s+", "", cleaned)
    if command is not None:
        cmd = command.strip()
        if cmd:
            lines = [line for line in cleaned.splitlines() if line.strip() != cmd]
            cleaned = "\n".join(lines)
...
def _write_to_child(master_fd: int, data: bytes) -> None:
    try:
        os.write(master_fd, data)
    except OSError as exc:
        if exc.errno in {5, 22}:
            return
        raise
...
def generate_transcript(story_path: Path, walkthrough_path: Path, out_path: Path) -> None:
    ...
    try:
        ...
        for command in commands:
            _append(parts, build_transcript_entry(command, ""))
            _write_to_child(master_fd, (command + "\n").encode("utf-8"))
            time.sleep(0.05)
            output = _read_available(master_fd, timeout=0.3)
            if output:
                _append(parts, _clean_output(output, command=command))

        _write_to_child(master_fd, b"quit\n")
        time.sleep(0.05)
        output = _read_available(master_fd, timeout=0.5)
        if output:
            _append(parts, _clean_output(output))
        if "Are you sure you want to quit?" in output:
            _write_to_child(master_fd, b"y\n")
            time.sleep(0.05)
            output = _read_available(master_fd, timeout=0.5)
            if output:
                _append(parts, _clean_output(output))
