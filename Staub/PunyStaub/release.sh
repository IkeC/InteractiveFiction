...
# Write .pi6rc with the PunyInformDE library path.
echo 'lib=/mnt/c/Source/PunyInformDE/lib/' > "$fiction_tools/.pi6rc"
...
WRAPPER="pi6"
TESTTERP="frotz"
cp -f staub.inf "$build_dir/staub.inf"
cp -f staub_ascii.inf "$build_dir/staub_ascii.inf"
cp -f "$build_dir/config.sh" "$build_dir/config.sh.orig"
tr -d '\r' < "$build_dir/config.sh.orig" > "$build_dir/config.sh"
trap "mv -f '$build_dir/config.sh.orig' '$build_dir/config.sh'; rm -f '$build_dir/staub.inf' '$build_dir/staub_ascii.inf' '$build_dir/staub_t.inf' '$build_dir/staub_t.z5' '$build_dir/gametext.txt' '$build_dir/abbrvs.h' '$build_dir/diskdefs'; rm -rf '$build_dir/Resources' '$build_dir/ascii_lib' '$build_dir/ascii_src' '$release_package_dir'" EXIT

# --- Populate build/Resources/ with loading-screen artwork --------------------
cover_src="$(dirname "$script_dir")/Inform7/Staub.materials/Cover.png"
...
with open(dst, 'wb') as handle:
	...
	handle.write(attrs)
PY
		if (cd "$tmpdir" && bash ./makescr.sh >/dev/null 2>&1) && [[ -f "$tmpdir/SCRLOAD.COM" ]]; then
...
sed -i '/^!% \$OMIT_SYMBOL_TABLE/d' staub.inf
if [[ "$build_debug" == false ]]; then
	sed -i '/^Constant DEBUG;$/d' staub.inf
	inform6_flags=(-S -e -c -E1)
else
	inform6_flags=(-SD -e -c -E1)
fi
punyinformde_win=$(wslpath -w /mnt/c/Source/PunyInformDE)
# Generate optimized abbreviations via transcript-format compile
echo '' > abbrvs.h
python3 -c "
with open('staub.inf') as f: c = f.read()
with open('staub_t.inf', 'w') as f: f.write('!% \$TRANSCRIPT_FORMAT=1\n' + c)"
"$inform6" "+include_path=${punyinformde_win}\\lib,${punyinformde_win}\\lib\\de" -v5 "${inform6_flags[@]}" -r staub_t.inf staub_t.z5 >/dev/null 2>&1 || true
rm -f staub_t.inf staub_t.z5
zabbrev -cu -v -i > abbrvs.h
rm -f gametext.txt
echo "Generated optimized abbreviations"
"$inform6" "+include_path=${punyinformde_win}\\lib,${punyinformde_win}\\lib\\de" -v5 "${inform6_flags[@]}" staub.inf staub.z5 >/dev/null

# --- Build ASCII version for retro platforms ---------------------------------
# Retro platforms cannot display high-ASCII characters (umlauts), so we
# preprocess all sources and library files to replace umlauts with digraphs
# (ae/oe/ue/ss) and compile a separate ASCII-clean story file.
echo "Building ASCII version (staub.ascii.z5)..."
punyinformde_src="/mnt/c/Source/PunyInformDE"
ascii_lib="$build_dir/ascii_lib"
ascii_src="$build_dir/ascii_src"
mkdir -p "$ascii_lib/de" "$ascii_src"
...
with open(sys.argv[2], 'w', encoding='utf-8') as f:
    f.write(text)" "$src" "$dst"
}

for f in globals_de.h chartable_de.h messages_de.h grammar_de.h parser_de.h article_de.h ext_talk_menu_inline_de.h; do
	convert_to_ascii "$punyinformde_src/lib/de/$f" "$ascii_lib/de/$f"
done
convert_to_ascii "$punyinformde_src/lib/puny.h" "$ascii_lib/puny.h"
convert_to_ascii "$build_dir/staub.inf"          "$ascii_src/staub.inf"
convert_to_ascii "$build_dir/abbrvs.h"           "$ascii_src/abbrvs.h"

# >abbrvs.h resolves to build/abbrvs.h (main file's dir); replace with ASCII version
cp "$ascii_src/abbrvs.h" abbrvs.h

ascii_lib_win=$(wslpath -w "$ascii_lib")
ascii_src_win=$(wslpath -w "$ascii_src")
"$inform6" "+include_path=${ascii_lib_win},${ascii_lib_win}\\de,${ascii_src_win},${punyinformde_win}\\lib,${punyinformde_win}\\lib\\de" -v5 "${inform6_flags[@]}" staub_ascii.inf staub.ascii.z5 >/dev/null
echo "Built staub.ascii.z5"

# Use the ASCII version for all retro platform builds.
# The original staub.z5 (Unicode) is preserved and restored after all builds.
cp staub.z5 staub.unicode.z5
cp staub.ascii.z5 staub.z5
echo "Using ASCII version (staub.z5) for retro platform builds."
...
if [[ -f "$speccy_cpm_base" && -f "$speccy_vezza" ]]; then
	...
	cp "$speccy_cpm_base" "$build_dir/staub_speccy.dsk"
	...
	for util in RPED.BAS BASIC.COM RAMDISC.FID DEVICE.COM DIR.COM \
		...
		idsk "$build_dir/staub_speccy.dsk" -r "$util" >/dev/null 2>&1 || true
	done
	# Add interpreter and story
	cp staub.z5 "$speccy_tmp/STORY.DAT"
	idsk "$build_dir/staub_speccy.dsk" -i "$speccy_tmp/PLAY.COM" -t 0 -f >/dev/null 2>&1
	idsk "$build_dir/staub_speccy.dsk" -i "$speccy_tmp/STORY.DAT" -t 0 -f >/dev/null 2>&1
	# Replace PROFILE.SUB to auto-run the interpreter
	idsk "$build_dir/staub_speccy.dsk" -r PROFILE.SUB >/dev/null 2>&1 || true
	if [[ -f "$build_dir/Resources/SCRLOAD.COM" ]]; then
		idsk "$build_dir/staub_speccy.dsk" -i "$build_dir/Resources/SCRLOAD.COM" -t 0 -f >/dev/null 2>&1
		printf 'scrload\r\nplay\r\n' > "$speccy_tmp/PROFILE.SUB"
	else
		printf 'play\r\n' > "$speccy_tmp/PROFILE.SUB"
	fi
	idsk "$build_dir/staub_speccy.dsk" -i "$speccy_tmp/PROFILE.SUB" -t 0 -f >/dev/null 2>&1
	rm -rf "$speccy_tmp"
...
# Restore original Unicode staub.z5 now that all retro builds are done.
mv staub.unicode.z5 staub.z5
...
if [[ "$build_release_zip" == true ]]; then
	...
	output_zip="$script_dir/Staub-Retro-Versions-${timestamp}.zip"
	# Remove any previously built release ZIPs.
	rm -f "$script_dir"/Staub-Retro-Versions-*.zip
...
Staub Retro Versions

This archive contains the retro target builds produced from PunyStaub.
...
EOF
	...
	for file in staub_speccy.dsk; do
		if [[ -f "$build_dir/$file" ]]; then
			cp -f "$build_dir/$file" "$release_package_dir/$file"
		fi
	done
	...
	if [[ -f "$build_dir/staub_apple2_s1.dsk" || -f "$build_dir/staub_apple2_s2.dsk" ]]; then
		mkdir -p "$release_package_dir/Apple2"
		for file in staub_apple2_s1.dsk staub_apple2_s2.dsk; do
			if [[ -f "$build_dir/$file" ]]; then
...
PY
		...
		retro_html="$script_dir/../Inform7/Staub.materials/Release/retro.html"
		if [[ -f "$retro_html" ]]; then
			newname=$(basename "$output_zip")
			# Replace any previous Staub-Retro-Versions*.zip filename with the new name.
			...
			sed -i -E 's/Staub-Retro-Versions[^"]*\.zip/'"$newname"'/g' "$retro_html" || true
